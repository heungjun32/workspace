...
def is_validated_morse_code(user_input):
    ...
    result = True
    validate_chars = ["-","."," "]
    validated_morse_codes = get_morse_code_dict().values()
    
    user_input2 = user_input[:]
    for code in validate_chars:
        user_input = user_input.replace(code,"")
    if len(user_input) > 0:
        result = False

    morse_codes = user_input2.split()
    for code in morse_codes:
        if code not in validated_morse_codes:
            result = False
...
def get_cleaned_english_sentence(raw_english_sentence):
    ...
    cleaned_sentence = raw_english_sentence.strip()
    sentence_mark = ".,!?"

    for mark in sentence_mark:
        cleaned_sentence = cleaned_sentence.replace(mark,"")
...
def decoding_character(morse_character):
    ...
    result = None
    for key,value in get_morse_code_dict().items():
...
def decoding_sentence(morse_sentence):
    ...
    for code in morse_sentence:
        if code in get_morse_code_dict().values():
            decoded_sentence += decoding_character(code)
        else:
            decoded_sentence += " "
...
def encoding_sentence(english_sentence):
    ...
    for mark in sentence_mark:
        english_sentence = english_sentence.replace(mark,"")
    changed_sentence = english_sentence
    changed_sentence = changed_sentence.upper()
    changed_sentence = list(changed_sentence)

    for code in changed_sentence:
        if code in alphabet:
            encoded_sentence += encoding_character(code)
            encoded_sentence += " "
        else:
            encoded_sentence += " "        
    encoded_sentence = encoded_sentence.strip()
    
    result = encoded_sentence
    return result
    # ==================================
...
def main():
    ...
    while True:
        ...
        if user_input == "0":
            ...
            encoding = encoding_sentence(user_input)
            print(encoding)
        elif is_validated_morse_code(user_input) == True:
            decoding = decoding_sentence(user_input)
            print(decoding)
        else:
            print("Wrong Input")
