...
def digits_to_words(input_string):
    ...
    * 만약 인풋 스트링에서 숫자가 존재하지 않는 다면, 빈 문자열 (empty string)을 반환함
    ...
    for i in input_string:
        if i.isdigit() ==True:
            digit_string=digit_string+number_list[int(i)]+" "
        else:
            pass
...
def to_camel_case(underscore_str):
    ...
    * 만약 어떤 변수 이름이 underscore로만 이루어 진다면, 빈 문자열만 반환해도 됨
    ...
    camelcase_str = camelcase_str.replace("_"," ").strip()

    result = ""

    is_prev_char_blank = False

    for char in camelcase_str:
        if char == " ":
            is_prev_char_blank = True
        else:
            if is_prev_char_blank:
                result += char.upper()
            else:
                result += char.lower()
            is_prev_char_blank = False
    ...
    result = to_camel_case(underscore_str)
    print(result)
